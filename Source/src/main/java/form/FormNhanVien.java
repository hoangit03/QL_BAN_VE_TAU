...
public class FormNhanVien extends javax.swing.JPanel {
	...
	public FormNhanVien(EntityManagerFactory emf) {
		this.emf = emf;
		initComponents();
	...
	private void initComponents() {
		...
		String[] tieuDe = { "Mã nhân viên", "CCCD", "Họ tên", "Ngày sinh", "Giới tính", "Địa chỉ", "Email",
...
//						"Số điện thoại", "Trạng thái" }));
	...
	private void btnTimActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnTimActionPerformed
		...
		if (!jtSDT.getText().trim().equalsIgnoreCase("")) {
			...
			JOptionPane.showMessageDialog(btnTim, "Chưa nhập tiêu chí tìm kiếm(CCCD,SĐT)", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	private void btnCapNhatActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnCapNhatActionPerformed
		...
		boolean gioiTinh = groupRadio.getSelection().equals(rdNam);
	...
	private void addDataTable(List<NhanVien> list) {
		...
		for(NhanVien nhanVien : list) {
			Object[] row = {nhanVien.getMaNhanVien(),nhanVien.getCccd(),nhanVien.getHoTen(),
							nhanVien.getNgaySinh().toString(),nhanVien.isGioiTinh() ? "Nam" : "Nữ"
							,nhanVien.getDiaChi(),nhanVien.getEmail(),
